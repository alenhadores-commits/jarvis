...
JARVIS_PROMPT_PATH = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
    "JARVIS_BASE",
    "PROMPT_JARVIS.txt",
)

def carregar_prompt_jarvis() -> str:
    try:
        with open(JARVIS_PROMPT_PATH, "r", encoding="utf-8-sig") as f:
            prompt = f.read().strip()
        if prompt:
            return prompt
    except Exception:
    ...
    return (
        ...
        "Evite introduções, repetições e explicações desnecessárias. "
        "Priorize a resposta útil."
    )

DIALOGUE_STYLE = carregar_prompt_jarvis()
...
def registrar_token_breakdown(
    provider: str,
    prompt_tokens: int,
    output_tokens: int,
):
    """Registra prompt/output sem alterar o total de tokens."""
...
def call_openai_provider(
    ...
    if response.status_code in (401, 403):
        ...
        raise RuntimeError(
            f"{cfg.name}: autenticaÃ§Ã£o recusada"
        )
    ...
    if response.status_code == 404:
        ...
        raise RuntimeError(
            f"{cfg.name}: modelo/endereÃ§o indisponÃ­vel"
        )
    ...
    if response.status_code >= 500:
        ...
        raise RuntimeError(
            f"{cfg.name}: servidor indisponÃ­vel"
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"{cfg.name}: resposta JSON invÃ¡lida"
        )
    ...
    # reasoning_tokens e um detalhamento de completion_tokens.
    # Nao somar novamente para evitar dupla contagem.
...
def call_gemini(
    ...
    if response.status_code == 404:
        ...
        raise RuntimeError(
            f"{cfg.name}: modelo indisponÃ­vel"
        )
...
class AIRouter:
    ...
    def chat(
        ...
        raise RuntimeError(
            "Todos os provedores disponÃ­veis falharam. "
            f"Ãšltimo erro: {last_error}"
        )
...
def check_router_key(authorization: str | None):
    ...
    if authorization != expected:
        raise HTTPException(
            status_code=401,
            detail="Router API key invÃ¡lida.",
        )
...
def chat_completions(
    ...
    if not request.messages:
        raise HTTPException(
            status_code=400,
            detail="messages nÃ£o pode ser vazio.",
        )
    ...
    return {
        ...
        "usage": (
            (
                lambda raw_usage: (
                    lambda prompt, completion, reasoning, total: {
                        ...
                        int(
                            ...
                            or total_message_tokens(messages)
                        ),
                        int(
                ...
                (
                    result.raw.get("usage") or {}
                    if isinstance(result.raw, dict)
                    else {}
                )
            )
            if isinstance(result.raw, dict)
            and result.raw.get("usage")
            else {
                "prompt_tokens": total_message_tokens(messages),
...
if __name__ == "__main__":
    ...
    if configured:
        ...
        print(
            "ATENÃ‡ÃƒO: nenhuma API configurada."
        )
        print(
