...
class IndexadorV2:
    ...
    def __init__(self, raiz_projeto=None):
        ...
        self.colecao_procedimentos = self.client.get_or_create_collection(
            name="jarvis_procedimentos",
            embedding_function=self.embedding
        )

        self.colecao_conhecimento = self.client.get_or_create_collection(
            name="jarvis_conhecimento",
            embedding_function=self.embedding
        )
    ...
    def _tipo_arquivo(self, caminho):
        ...
        caminho_str = str(caminho).lower().replace("\\", "/")

        # Arquivos que nao devem entrar no RAG
        if "dados_chromium" in caminho_str:
            return None

        if "extensions_crx_cache" in caminho_str:
            return None
        ...
        if nome == "dados_identidade.json":
            return "memorias"

        # Habilidades executaveis
        if nome == "habilidades.json" and "habilidades" in caminho_str:
            return "procedimentos"

        # Prompt e conhecimento estrutural do JARVIS
        if nome == "prompt_jarvis.json":
            return "conhecimento"
    ...
    def _colecao(self, tipo):
        ...
        if tipo == "procedimentos":
            return self.colecao_procedimentos

        if tipo == "conhecimento":
            return self.colecao_conhecimento
